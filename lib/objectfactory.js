...
var property = function(target, source, k, data, path) {
	var newValue, type = typeof source[k];
	var onDelete = data.onDelete[path+k];
	data.onDelete[path+k] = undefined;

	newValue = type === strFunction && source[k].hash === hash ?
		source[k].call(undefined, target, k, path+k, data):
		source[k];

	// Call onDelete hook for this value
	if(data.decorated)
		callHooks(onDelete, [target, k, path+k, data.shared], data);
...
var build = function(modules, data){
	...
	// call first module decorators
	if(data.decorated)
		for(var key in instance)
			if(typeof instance[key] === strFunction && instance[key].hash === hash) {
				onDelete = data.onDelete[key];
				data.onDelete[key] = undefined;
				value = instance[key].call(undefined, data.instance, key, key, data);
				callHooks(onDelete, [data.instance, key, key, data.shared], data);
				instance[key] = value;
			}
